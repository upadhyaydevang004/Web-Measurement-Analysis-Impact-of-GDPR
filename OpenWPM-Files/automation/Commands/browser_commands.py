...
def dump_page_source(visit_id, driver, manager_params, suffix=''):
    ...
    print("\n" + driver.current_url.encode('utf-8'))
    
    site = driver.page_source.encode('utf8')
    
    dataprotectionofficer = ['Data Protection Officer', 'data protection officer','Datenschutzbeauftragten']	
    for word in dataprotectionofficer:
        if word in site:
           print("Data Protection Officer : FOUND")
           break
    
    regex = '^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$'
    site = driver.page_source.encode('utf8')	
    if(re.search(r'[\w\.-]+@[\w\.-]+', site)):
        print("Email id found") 

    rightToRectification = ['right to rectification', 'rectification', 'Berichtigung' , 'berichtigung', 'Recht auf Berichtigung', 'recht auf berichtigung']	
    for word in rightToRectification:
        if word in site:
           print("Right to Rectification : FOUND")
           break

    dataPortability = ['data portability','Data Portability','Datenübertragbarkeit','datenübertragbarkeit','Recht auf Datenübertragbarkeit','recht auf datenübertragbarkeit']
    for word in dataPortability:
        if word in site:
           print("data portability : FOUND")
           break    

    rightToDeletion = ['right to deletion' , 'Right to Deletion' , 'deletion', 'Recht auf Löschung' , 'recht auf löschung' , 'right to be forgotten' , 'Right to be Forgotten' , 'Recht, vergessen zu werden' , 'recht, vergessen zu werden', 'Streichung' , 'streichung']
    for word in rightToDeletion:
        if word in site:
           print("Right To Deletion : FOUND")
           break

    rightToObjection = ['right to objection' , 'right of objection' , 'right to object', 'Right to Object' , 'Right of Objection' , 'Right to Objection', 'object', 'Object','Objection','objection', 'Widerspruchsrecht' , 'widerspruchsrecht', 'Objekt' , 'objekt', 'Einwand', 'einwand', 'einwande' , 'Einwande']

    for word in rightToObjection:
        if word in site:
           print("Right To Objection : FOUND")
           break

    legitimateInterest = ['legitimate interest','Legitimate Interest', 'Legitimate interest', 'berechtigtes Interesse']
    for word in legitimateInterest:
        if word in site:
           print("Legitimate Interest: FOUND")
           break
